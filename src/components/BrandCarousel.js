      ...
      >
        {sorted.map((brand) => (
          <SwiperSlide key={brand.id}>
            {/* Aumentamos la altura aquí (ej. h-32, h-28, h-24) */}
            <div className="flex justify-center items-center h-32 sm:h-28 md:h-24 transition-all duration-300 cursor-pointer hover:scale-105">
              {/* Añadimos un ancho máximo si es necesario, ej. w-40 */}
              <div className="relative w-[200px] h-full">
                <Image
                  src={getCloudinaryUrl(brand.logoUrl, {
                    width: 250,
                    height: 100,
                  })}
                  alt={brand.name}
